...
Typescript is transformed into javascript with the `esbuild` tool, which was chosen for its speed. During production build, `esbuild` adds hashes to the names of the output files for reliable caching and cache busting. See `portal-static/utils/build.js` for the options of the tool, and list of typescript files that are used as entry points.
